...
def test_git_helpers_detach_stdin_and_keep_timeouts(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert staleness.commits_touching_since("README.md", "2026-01-01T00:00:00Z", tmp_path) == 1
...
def _call_mcp_tool(
    ...
    def read_response(message_id: int, wait_seconds: float) -> dict[str, Any] | None:
...
def test_mcp_stdio_precheck_file_returns_with_git_backed_analysis(
    ...
    append_event(
        Event(type="note", summary="README precheck regression fixture", location="README.md"),
        project,
    )
...
def test_mcp_stdio_add_note_returns_and_writes_event(
    ...
    events_text = (project / ".projectmem" / "events.jsonl").read_text(encoding="utf-8")
