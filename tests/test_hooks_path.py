...
def _bash_failure_reason(result: subprocess.CompletedProcess[str]) -> str:
    ...
    return ((result.stderr or "") + "\n" + (result.stdout or "")).replace("\x00", "").strip()
...
def test_hook_runs_under_stripped_path(tmp_path: Path, fake_pjm: Path) -> None:
    ...
    # Resolve the test harness shell before stripping PATH; the stripped PATH is
    # the condition being tested inside the hook process.
    result = subprocess.run(
        [bash, str(hook)],
        cwd=repo,
        env=minimal_env,
